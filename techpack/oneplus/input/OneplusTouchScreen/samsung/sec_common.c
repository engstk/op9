...
static int tp_auto_test_read_func(struct seq_file *s, void *v)
{
	...
	//init syna_testdata
	sec_testdata.fd = ts->result_data;
	sec_testdata.length = ts->result_max_len;
	sec_testdata.pos = &ts->result_cur_len;
...
static int tp_auto_test_result(struct seq_file *s, void *v)
{
	...
	if (ts->result_flag) {
		if (ts->result_data) {
			if (ts->result_cur_len) {
				seq_write(s, ts->result_data, ts->result_cur_len);
			}

			kfree(ts->result_data);
			TPD_INFO("%s:free data ok\n", __func__);
			ts->result_flag = 0;
		}

	} else {
		TPD_INFO("%s:it must do auto test frist\n", __func__);
...
static int get_firmware_version_read(struct seq_file *s, void *v)
{
	struct touchpanel_data *ts = s->private;

	if (!ts->ts_ops->get_ic_firmware_version) {
		TPD_INFO("%s:not support\n", __func__);
		seq_printf(s, "not support\n");
	} else {
		ts->ts_ops->get_ic_firmware_version(ts->chip_data, &ts->panel_data);
...
static int proc_get_firmware_version_open(struct inode *inode, struct file *file)
{
	return single_open(file, get_firmware_version_read, PDE_DATA(inode));
}
...
int sec_create_proc(struct touchpanel_data *ts, struct sec_proc_operations *sec_ops)
{
	...
	prEntry_tmp = proc_create_data("baseline_test_result", 0666, ts->prEntry_tp, &tp_auto_test_result_fops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}
	...
	prEntry_tmp = proc_create_data("get_ic_firmware_version", 0666, ts->prEntry_tp, &proc_get_firmware_version_fops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}
