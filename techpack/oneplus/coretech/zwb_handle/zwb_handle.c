	...
	unsigned long end, struct mm_walk *walk)
{
	...
	for (index = start; index != end; index += PAGE_SIZE) {
		...
		if (!is_fast_entry(entry)) {
			...
			blk_queue_enter(bdev->bd_queue, 0);
...
static const struct mm_walk_ops clear_walk_ops = {
	.pmd_entry = zwb_clear_walk_pmd_entry,
};
...
retry:
	...
	for (vma = mm->mmap; vma; vma = vma->vm_next) {
		...
		err = walk_page_range(vma->vm_mm, vma->vm_start, vma->vm_end, &clear_walk_ops, vma);
		if (err == -1)
			break;
	}
...
static inline bool should_skip(const char *comm)
{
	...
	return strncmp("eplus.wallpaper", comm, TASK_COMM_LEN)
		&& strncmp("ndroid.systemui", comm, TASK_COMM_LEN);
}
