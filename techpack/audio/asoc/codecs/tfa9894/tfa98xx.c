...
{
#ifdef TFA98XX_USE_DEBUG_FS
	struct i2c_client *i2c = file->private_data;
#else /*TFA98XX_USE_DEBUG_FS*/
	struct i2c_client *i2c = PDE_DATA(file_inode(file));
#endif/*TFA98XX_USE_DEBUG_FS*/
...
int tfa98xx_keyreg_print(struct tfa98xx *tfa98xx)
{
       ...
       if (ret < 0) {
              dev_dbg(&tfa98xx->i2c->dev, "Failed to read 0-SYS_CONTROL0\n", ret);
              return -EIO;
       }

       ret = regmap_read(tfa98xx->regmap, 0x10, &STATUS_FLAGS0);
       if (ret < 0) {
              dev_dbg(&tfa98xx->i2c->dev, "Failed to read 10h-STATUS_FLAGS0\n", ret);
              return -EIO;
       }

       ret = regmap_read(tfa98xx->regmap, 0x11, &STATUS_FLAGS1);
       if (ret < 0) {
              dev_dbg(&tfa98xx->i2c->dev, "Failed to read 11h-STATUS_FLAGS0\n", ret);
              return -EIO;
       }

       ret = regmap_read(tfa98xx->regmap, 0x13, &STATUS_FLAGS3);
       if (ret < 0) {
              dev_dbg(&tfa98xx->i2c->dev, "Failed to read 13h-STATUS_FLAGS3\n", ret);
              return -EIO;
       }

       ret = regmap_read(tfa98xx->regmap, 0x14, &STATUS_FLAGS4);
       if (ret < 0) {
              dev_dbg(&tfa98xx->i2c->dev, "Failed to read 14h-STATUS_FLAGS4\n", ret);
              return -EIO;
       }

       ret = regmap_read(tfa98xx->regmap, 0x6e, &STATUS_FLAGS5);
       if (ret < 0) {
              dev_dbg(&tfa98xx->i2c->dev, "Failed to read 6eh-STATUS_FLAGS5\n", ret);
              return -EIO;
       }
...
static ssize_t tfa98xx_regmap_read_registers(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i <= tfa98xx_regmap.max_register; i++) {
		/* Format the register */
		len = sprintf(buf + buf_pos, "%.2x: ", i);
		buf_pos += len;

		/* Format the value, write all X if we can't read */
		ret = regmap_read(tfa98xx->regmap, i, &val);
		if (ret == 0)
			sprintf(buf + buf_pos, "%.4x", val);
		else
			memset(buf + buf_pos, 'X', 4);
		buf_pos += 4;
...
static ssize_t tfa98xx_regmap_write_registers(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_err("%s: reg: %2x, value: %4x\n", __func__, reg, value);
	ret = regmap_write(tfa98xx->regmap, reg, value);
	if (ret < 0) {
		pr_err("%s: regmap_write return error(%d)\n", __func__, ret);
		return 0;
	}

	return count;
}
static struct device_attribute tfa98xx_registers_attr =
	__ATTR(registers, 0664, tfa98xx_regmap_read_registers, tfa98xx_regmap_write_registers);
...
static struct device_attribute tfa98xx_state_attr =
     __ATTR(calibra, 0664, tfa98xx_state_show, tfa98xx_state_store);
...
// modify end
	...
	if (!dummy) {
		...
		ret = sysfs_create_file(&i2c->dev.kobj, &tfa98xx_registers_attr.attr);
		if(ret < 0)
		{
			pr_err("%s sysfs_create_file tfa98xx_registers_attr err.", __func__);
		}
	}
