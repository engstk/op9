...
static inline int gc_perf_ratio(struct f2fs_sb_info *sbi)
{
	...
	return reclaimable_user_blocks == 0 ? 100 :
			100ULL * free_user_blocks(sbi) /
			reclaimable_user_blocks;
}

static inline bool is_invalid_blocks_enough(struct f2fs_sb_info *sbi)
{
	block_t reclaimable_user_blocks = sbi->user_block_count -
						written_block_count(sbi);
...
static inline void of2fs_tune_wait_ms(struct f2fs_sb_info *sbi,
...
static inline bool of2fs_gc_wait(struct f2fs_sb_info *sbi,
		wait_queue_head_t *wq, unsigned int *wait_ms)
{
	struct f2fs_gc_kthread *gc_th = sbi->gc_thread;
				...
				struct gc_inode_list *gc_list, int gc_type)
{
	...
	for (segno = start_segno; segno < end_segno; segno++) {
		...
		if (gc_type == BG_GC || get_valid_blocks(sbi, segno, 1) == 0) {
			if (type == SUM_TYPE_NODE)
				bd_inc_array_val(sbi, gc_node_segments,
					gc_type, 1);
			else
				bd_inc_array_val(sbi, gc_data_segments,
					gc_type, 1);
			bd_inc_array_val(sbi, hotcold_gc_segments,
					hc_type + 1, 1);
		}
